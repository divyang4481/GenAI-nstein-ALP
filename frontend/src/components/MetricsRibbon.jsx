...
export default function MetricsRibbon({ metrics }) {
  const items = [
    { label: "At-risk orders", value: metrics?.at_risk_orders || 6, note: "2 critical", icon: AlertTriangle, tone: "text-[#C4314B]", bg: "bg-rose-50" },
    { label: "SLA breaches in 24h", value: 3, note: "1 within 4 hours", icon: Clock3, tone: "text-[#C68A00]", bg: "bg-amber-50" },
    { label: "Pending approvals", value: metrics?.pending_human_approvals || 4, note: "Oldest 21 min", icon: UserCheck, tone: "text-[#0F6CBD]", bg: "bg-blue-50" },
    { label: "Value protected", value: `R$ ${(metrics?.estimated_retention_roi_brl || 740).toLocaleString("pt-BR", {minimumFractionDigits: 2})}`, note: "Today", icon: CircleDollarSign, tone: "text-[#1A7F37]", bg: "bg-emerald-50" }
  ];
